...
**1. 正本データはJEPXにあります。** リポジトリが持っている生CSVは**当年度分だけ**です（過去年度分は `monthly.json` に月平均としてだけ残っています）。過去データを直すときは、JEPXから取り直してください（→ [データ方針](ops/architecture.md#6-データ方針過去データを推測で埋めない)）。
